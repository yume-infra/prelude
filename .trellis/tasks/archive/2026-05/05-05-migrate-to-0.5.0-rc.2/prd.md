...
6. **Skill paths in existing task jsonl files**: if the user has active tasks with `implement.jsonl` / `check.jsonl` referencing old paths (e.g. `.agents/skills/check/SKILL.md` without `trellis-`), update those rows to the new `trellis-*` skill paths and run `task.py validate <task-dir>`.
