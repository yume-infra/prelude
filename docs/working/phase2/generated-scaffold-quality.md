...
6. M006 已把 package manifest policy 迁移为 owner-owned package manifest contributions。后续 package 质量问题应先查看 `package-manifest-contributions.ts`、`package-json.ts` 与对应 owner contribution API；不要回退到中心 package policy 表或模板 helper。

已关闭的策略边界如下：

- 不要在没有新 smoke failure 的情况下重开 React Router 静态 import 策略。
- 不要在没有新 smoke failure 的情况下重开 package manifest / TypeScript config 的 JSON source-order 策略。package manifest 的后续扩展应走 M006 contribution collector；TypeScript config 仍是 deferred hotspot watchlist，除非多个 owner 开始共同修改 compiler options、references、paths 或 types。
