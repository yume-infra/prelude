...
## Source map：generated finding → durable owner

本节是 `T02` 的交付物。它把 `T01` 的运行时证据映射回 durable 修复入口，供后续 slice 直接分流。这里不修改模板、package mutation 或 lint policy；只记录最小可行动的源头、风险与下游归属。

| Generated file / command surface | Observed targets | Category | Likely durable source owner / path | Repair shape | Risk / cost | Downstream slice |
|---|---|---|---|---|---|---|
| `pnpm lint --max-warnings=0` exits 254 because `lint` script is absent; no generated source file locator exists. | `react-minimal`, `vue-minimal` | `generated config policy` | `apps/cli/src/core/workspace-bootstrap.ts` controls `lint` / `lint:fix` scripts when `linting === 'antfu-eslint'`; preset defaults in frontend family currently allow minimal presets to be generated without an auditable lint script. | Decide whether every supported preset must emit a lint script, whether the audit gate should skip lint-disabled presets, or whether minimal presets should change defaults. | Rethink-required policy boundary; do not fix as template whitespace. | `S03` for policy, then `S04` if smoke gates are tightened. |
| `.vscode/settings.json` missing EOF newline. | `react-full`, `vue-full`, linked full examples | `template whitespace` | `apps/cli/templates/fragments/common/linter/vscode.settings.json.hbs`, registered by `workspaceBootstrapTemplates` as WorkspaceBootstrap fragment render. | Add durable final newline / render normalization for generated editor settings. | Mechanical; low risk, but keep it separate from editor-only diagnostics. | `S02` |
| `README.md` missing EOF newline. | `react-full`, `vue-full`, linked full examples | `template whitespace` | `apps/cli/templates/fragments/common/README.md.hbs`, registered by shared frontend finishing templates. | Add durable final newline / render normalization for generated README. | Mechanical; low risk. | `S02` |
| `eslint.config.mjs` indentation, trailing spaces, and missing trailing comma. | `react-full`, `vue-full`, linked full examples | `template whitespace` | `apps/cli/templates/fragments/common/linter/eslint.config.mjs.hbs`, registered by WorkspaceBootstrap lint templates. | Format the template under the generated ESLint rules instead of relying on `--fix`. | Mechanical; low risk. | `S02` |
| `vite.config.ts` import ordering, plugin-array whitespace, trailing spaces, and missing trailing comma. | `react-full`, `vue-full`, linked full examples | `template whitespace` | `apps/cli/templates/fragments/common/vite.config.ts.hbs`, registered by shared frontend core templates. | Reorder conditional imports/plugins and remove whitespace/comma violations in the template. | Mostly mechanical; first fixer should verify Handlebars branch output for React, Vue, Tailwind and non-Tailwind combinations. | `S02` |
| `src/stores/counter.ts` React import/newline style. | `react-full` | `template whitespace` | `apps/cli/templates/fragments/react/Counter.ts.hbs`, owned through `ReactCounterStoreTemplate` / State Management capability. | Add the required blank line after imports in each active state-management branch. | Mechanical; low risk. | `S02` |
| `src/stores/counter.ts` Vue import ordering, named import ordering, and missing trailing comma. | `vue-full` | `template whitespace` | `apps/cli/templates/fragments/vue/counter-store.ts.hbs`, owned through `VueCounterStoreTemplate` / State Management capability. | Reorder imports/named imports and add the returned-object trailing comma. | Mechanical; low risk. | `S02` |
| `src/components/Counter.tsx` JSX parentheses line-break style. | `react-full-linked` excerpt; same family expected in React full lint output | `template whitespace` | `apps/cli/templates/fragments/react/Counter.tsx.hbs` plus `apps/cli/templates/partials/react/counter-body.hbs`. | Format JSX return shape in the fragment/partial so generated JSX satisfies Antfu stylistic rules. | Mechanical, but source is ambiguous between fragment and partial; first inspection target is the partial because it owns the JSX body. | `S02` |
| `src/routes/router.tsx` in the audit evidence / React router module fails `react-refresh/only-export-components`. Current source registry maps the same router template to `src/router/index.tsx`, so fixers should verify generated target naming before editing. | `react-full`, `react-full-linked` | `framework lint semantics` | `apps/cli/templates/fragments/react/router.ts.hbs`, registered by `ReactRouterIndexTemplate` / Router capability owner. | Decide whether to split router creation from component exports, rename/move route modules, or add a scoped lint override for generated router files. | Rethink-required framework semantics; not a whitespace fix. | `S03` |
| `src/App.vue` `RouterLink` single-line content warning. | `vue-full`, `vue-full-linked` | `framework lint semantics` | `apps/cli/templates/fragments/vue/App.vue.hbs`, registered by Vue family templates. | Either reflow router links to Vue lint-preferred multiline shape or adjust Vue template readability policy. | Mixed: likely mechanical, but keep policy escape hatch if generated readability worsens. | `S02` first; escalate unresolved rule/readability conflict to `S03`. |
| `src/views/Home.vue` Vue indentation, template block newline, trailing spaces, and style block newline. | `vue-full`, `vue-full-linked` | `template whitespace` / `framework lint semantics` | `apps/cli/templates/fragments/vue/Home.vue.hbs` plus `apps/cli/templates/partials/vue/style.hbs`. | Fix indentation and blank lines in the view fragment; check whether the style partial should include a newline after `<style scoped...>`. | Mostly mechanical; source is split between view fragment and partial, so first fixer should inspect both. | `S02`; policy conflict, if any, to `S03`. |
| `src/views/About.vue` unused `Counter` import and style-block newline. | `vue-full`, `vue-full-linked` | `unused imports / dead code` and `template whitespace` | `apps/cli/templates/fragments/vue/About.vue.hbs`, registered as `VueRouterAboutTemplate` / Router capability owner; style opening tag comes from `apps/cli/templates/partials/vue/style.hbs`. | Remove unused import or render matching usage; add required style-block newline through the fragment or partial. | Mechanical for unused import; style partial affects other Vue SFCs, so verify all Vue branches. | `S02` |
| `package.json` JSON key ordering. | `react-full`, `vue-full`, linked full examples | `generated config policy` | `apps/cli/src/core/modifier/package-json.ts` calls `entry.sortKeys(true)` after base and owner mutations; `apps/cli/src/core/workspace-bootstrap.ts` contributes lint scripts/deps. | Decide whether generated JSON should follow lint sort order, conventional package order, or generated-writer normalization before lint. | Rethink-required because package ordering is a generated JSON policy, not a local template typo. | `S03` |
| `tsconfig.json`, `tsconfig.app.json`, `tsconfig.node.json` JSON key ordering. | `react-full`, `vue-full` | `generated config policy` | `apps/cli/templates/fragments/common/ts/*.hbs`, registered by shared frontend core templates; JSON ordering may also be affected by broader generated config policy. | Decide whether to hand-sort these JSON templates or introduce a generated JSON normalization step aligned with lint. | Strategy-level if normalized globally; mechanical only if policy chooses hand-sorted templates. | `S03` decision; potential follow-up implementation in `S02` if scoped to hand sorting. |
| Tailwind/lightningcss unknown at-rule warnings during `pnpm build`; no generated source line is stable in lint output. | `react-full`, `vue-full`, linked smoke build stderr | `dependency/build warning` | `apps/cli/templates/fragments/common/css/style.css.hbs` emits `@import "tailwindcss"`; `apps/cli/src/core/modifier/package-json.ts` pins Tailwind/Vite dependencies; build policy sits with shared frontend CSS/Vite capability. | Determine whether warnings are acceptable dependency noise, require CSS/Tailwind config changes, or require dependency version/pipeline adjustment. | Rethink-required; build exits 0, so do not conflate with lint failure or `D011` 0-warning lint policy. | `S03` |
| `smoke:examples` passes while generated linked projects fail later lint. | `react-full-linked`, `vue-full-linked` | `generated config policy` / audit gate policy | linked smoke command flow, not a generated file; first inspection target is the create-yume smoke script/test harness that generates linked examples. | Decide whether smoke should include `lint --max-warnings=0`, keep build-only smoke, or add a separate generated-lint audit command. | Strategy/gate decision; downstream gate implementation belongs outside template cleanup. | `S04`; vocabulary/evidence shape to `S05`. |
| No standalone editor-only diagnostics were observed. `.vscode/settings.json` appears as a generated JSON lint target, not as IDE-only runtime diagnostics. | all audited outputs | `editor-only diagnostics` absent | N/A | Preserve this distinction in future audit vocabulary so editor settings lint failures are not mislabeled as editor-only diagnostics. | Documentation/process only. | `S05` |

### Downstream handoff summary

- `S02` should start with mechanical source edits: EOF/newline fixes, ESLint/Vite template formatting, React/Vue store import ordering, Vue SFC whitespace, unused Vue import, and JSX return formatting. These are low-cost because the generated-file locator maps to a concrete template fragment or partial.
- `S03` should resolve strategy findings before implementation: minimal-preset lint policy, generated JSON ordering policy, React Refresh router semantics, Tailwind/lightningcss build-warning policy, and any Vue rule/readability conflict that cannot remain a mechanical template edit.
- `S04` should treat linked smoke as a gate-design issue: current smoke proves generated projects build, but the separate lint evidence proves build-only smoke is insufficient for lint cleanliness.
- `S05` should reuse this source map as audit workflow vocabulary and examples: `generated file`, `command/tooling output`, `durable source owner`, `classification`, `repair shape`, `mechanical`, `rethink-required`, and `downstream slice`.

### Source-map negative-test handling

- Malformed locator handling: missing `lint` scripts and Tailwind/lightningcss warnings do not identify a generated source file, so they are classified as command/tooling surfaces rather than assigned invented file paths.
- Ambiguous owner handling: JSX counter formatting and Vue SFC style newline findings list both the fragment and partial owners, plus the first inspection target.
- Boundary handling: lint failures remain separate from build warnings; `dependency/build warning` findings must not be merged into `generated config policy` or the `lint --max-warnings=0` failure bucket.
