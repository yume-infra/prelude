...
const cliRoot = join(import.meta.dirname, '../apps/cli')
...
try {
  ...
  await mkdir(join(target, '.prelude/alpha/feedback'), { recursive: true }); await mkdir(join(target, '.prelude/beta/feedback'), { recursive: true })
  ...
  const alphaFeedback = join(target, '.prelude/alpha/feedback/note.md'); const betaFeedback = join(target, '.prelude/beta/feedback/note.md')
  ...
  assert.equal((await readdir(join(target, '.prelude/alpha'))).includes('managed'), false); assert.equal((await readdir(join(target, '.prelude/alpha'))).includes('repos'), false); assert.equal(await readFile(alphaFeedback, 'utf8'), 'target-owned evidence before apply\n')
  ...
  assert.equal(await readFile(join(target, '.prelude/alpha/managed/alpha.txt'), 'utf8'), alphaV1.managedText); assert.equal(await readFile(join(target, '.prelude/alpha/repos/AGENTS.md'), 'utf8'), alphaV1.pinnedText); assert.equal(await readlink(join(target, '.prelude/alpha/repos/CLAUDE.md')), 'AGENTS.md')
  ...
  const alphaRepos = join(target, '.prelude/alpha/repos'); await rm(join(alphaRepos, 'CLAUDE.md')); await writeFile(join(alphaRepos, 'CLAUDE.md'), 'target edit\n'); await writeFile(join(alphaRepos, 'AGENTS.md'), 'target edit\n'); await writeFile(join(alphaRepos, 'target-only.txt'), 'remove me\n')
  ...
  for (const integrationId of ['alpha', 'beta']) assert.deepEqual((await readdir(join(target, `.prelude/${integrationId}`))).sort(), ['feedback', 'managed', 'repos'])
  const preludeEntries = [await readdir(join(target, '.prelude')), await readdir(join(target, '.prelude/alpha')), await readdir(join(target, '.prelude/beta'))].flat(); assert.equal(preludeEntries.some(entry => /receipt|manifest|journal|snapshot|provider|dispatcher|\.git/i.test(entry)), false)
