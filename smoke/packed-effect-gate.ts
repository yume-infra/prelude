...
const scripts = {
  typecheck: 'node run-tsgo.mjs --noEmit --project tsconfig.json',
  lint: 'eslint src',
...
const tsgoRunner = `import { spawnSync } from 'node:child_process'
import process from 'node:process'

const resolved = spawnSync('effect-tsgo', ['get-exe-path'], { encoding: 'utf8' })
const executable = (resolved.stdout ?? '').trim()
if (resolved.status !== 0 || executable === '') {
  process.stderr.write(resolved.stderr || 'effect-tsgo did not return an executable path\\n')
  process.exit(resolved.status && resolved.status > 0 ? resolved.status : 1)
}
const result = spawnSync(executable, process.argv.slice(2), { stdio: 'inherit' })
process.exit(result.status && result.status > 0 ? result.status : 0)
`
...
async function writeSelectedPackage(root: string, name: string) {
  ...
  await writeFile(join(root, 'run-tsgo.mjs'), tsgoRunner)
  await writeFile(join(root, 'src/index.ts'), source)
}

async function installTarget(root: string) {
  await execa('pnpm', ['install', '--reporter', 'append-only'], {
    cwd: root,
...
async function runGate(name: 'single' | 'monorepo', packageRoots: ReadonlyArray<string>, integrationId: string) {
  ...
  if (packageRoots.includes('.')) {
    ...
    await writeFile(join(target, 'run-tsgo.mjs'), tsgoRunner)
