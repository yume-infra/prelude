...
`)
  ...
  const initialCheck = yield* cliRun(['check', '--json'], target, false)
  assert.notEqual(initialCheck.exitCode, 0)
  const initialFailure = parseCliJson(initialCheck, cli, ['check', '--json'], target)
  assert.equal(initialFailure.schemaVersion, 1)
  assert.equal(initialFailure.result, 'error')
  assert.equal(initialFailure.error.code, 'non-converged')
  assert.equal(initialFailure.error.plan.executionHash, plan.executionHash)
  assert.equal(initialFailure.error.plan.outputs.some((output: Json) => output.status === 'change'), true)
  assert.equal(initialFailure.error.plan.outputs.some((output: Json) => output.resolvedPath === '.prelude/alpha/managed'), true)
  assert.equal(initialFailure.error.plan.outputs.some((output: Json) => output.resolvedPath === '.prelude/alpha/repos'), true)
  assert.equal(initialFailure.error.plan.requirements.some((requirement: Json) => Boolean(requirement.satisfied) === false), true)
  const initialHumanCheck = yield* cliRun(['check'], target, false)
  assert.notEqual(initialHumanCheck.exitCode, 0)
  assert.match(initialHumanCheck.stderr, /Execution hash:/)
  assert.match(initialHumanCheck.stderr, /\.prelude\/alpha\/(managed|repos)/)
  assert.match(initialHumanCheck.stderr, /Unsatisfied Requirements:/)
  assert.match(initialHumanCheck.stderr, /alpha\/required-packages-app -> packages\/app: @synthetic\/required 1\.0\.0/)
  ...
  yield* fs.writeFileString(join(target, '.prelude/alpha/managed/alpha.txt'), 'managed drift\n')
  const managedDriftCheck = yield* cliRun(['check', '--json'], target, false)
  assert.notEqual(managedDriftCheck.exitCode, 0)
  const managedDriftFailure = parseCliJson(managedDriftCheck, cli, ['check', '--json'], target)
  assert.equal(managedDriftFailure.error.code, 'non-converged')
  assert.equal(managedDriftFailure.error.plan.outputs.some((output: Json) => output.declaration.id === 'managed' && output.status === 'change' && output.resolvedPath === '.prelude/alpha/managed'), true)
  const managedRepair = parseJson((yield* cliRun(['apply', '--plan-hash', managedDriftFailure.error.plan.executionHash, '--json'])).stdout)
  ...
  const pinnedDriftCheck = yield* cliRun(['check', '--json'], target, false)
  assert.notEqual(pinnedDriftCheck.exitCode, 0)
  const pinnedDriftFailure = parseCliJson(pinnedDriftCheck, cli, ['check', '--json'], target)
  assert.equal(pinnedDriftFailure.error.code, 'non-converged')
  assert.equal(pinnedDriftFailure.error.plan.outputs.some((output: Json) => output.declaration.id === 'pins' && output.status === 'change' && output.resolvedPath === '.prelude/alpha/repos'), true)
  ...
  yield* fs.remove(alphaFeedback, { force: true })
  const issueCheck = yield* cliRun(['check', '--json'], target, false)
  assert.notEqual(issueCheck.exitCode, 0)
  const issueFailure = parseCliJson(issueCheck, cli, ['check', '--json'], target)
  assert.equal(issueFailure.error.code, 'non-converged')
  assert.equal(issueFailure.error.plan.issues.some((issue: Json) => issue.declaration.id === 'target-observation'), true)
  const issueHumanCheck = yield* cliRun(['check'], target, false)
  assert.notEqual(issueHumanCheck.exitCode, 0)
  assert.match(issueHumanCheck.stderr, /Issues:/)
  assert.match(issueHumanCheck.stderr, /alpha\/target-observation: Target roots or feedback are not observable/)
  yield* fs.writeFileString(alphaFeedback, 'target-owned evidence after apply\n')
  const betaModulePath = join(target, 'node_modules/@synthetic/beta/prelude.js')
  const betaModule = yield* fs.readFileString(betaModulePath)
  yield* fs.writeFileString(betaModulePath, betaModule.replace('const name = "beta"', 'const name = "alpha"').replace('const policyFile = "beta-settings.json"', 'const policyFile = "settings.json"'))
  const conflictCheck = yield* cliRun(['check', '--json'], target, false)
  assert.notEqual(conflictCheck.exitCode, 0)
  const conflictFailure = parseCliJson(conflictCheck, cli, ['check', '--json'], target)
  assert.equal(conflictFailure.error.code, 'non-converged')
  assert.equal(conflictFailure.error.plan.conflicts.some((conflict: Json) => conflict.kind === 'jsonKeyIdentity'), true)
  assert.match(conflictCheck.stderr, /jsonKeyIdentity/)
  const conflictHumanCheck = yield* cliRun(['check'], target, false)
  assert.notEqual(conflictHumanCheck.exitCode, 0)
  assert.match(conflictHumanCheck.stderr, /Conflicts:/)
  assert.match(conflictHumanCheck.stderr, /jsonKeyIdentity.*alpha\/policy-root.*beta\/policy-root/)
  yield* fs.writeFileString(betaModulePath, betaModule)
  ...
  const failedCheckEnvelope = parseCliJson(failedCheck, cli, ['check', '--json'], target)
  assert.equal(failedCheckEnvelope.schemaVersion, 1)
  assert.equal(failedCheckEnvelope.result, 'error')
  assert.equal(failedCheckEnvelope.error.code, 'checks-failed')
  assert.equal(failedCheckEnvelope.error.planPhase, 'after-checks')
  assert.equal(failedCheckEnvelope.error.plan.converged, true)
  assert.equal(failedCheckEnvelope.error.checks.some((check: Json) => check.checkId === 'a-missing' && check.exitCode !== 0), true)
  assert.equal(failedCheckEnvelope.error.checks.some((check: Json) => check.checkId === 'z-after' && check.exitCode === 0), true)
  ...
  const requirementCheck = yield* cliRun(['check', '--json'], target, false)
  assert.notEqual(requirementCheck.exitCode, 0)
  const requirementFailure = parseCliJson(requirementCheck, cli, ['check', '--json'], target)
  assert.equal(requirementFailure.error.code, 'non-converged')
  assert.equal(requirementFailure.error.plan.requirements.some((requirement: Json) => Boolean(requirement.satisfied) === false && requirement.declaration.packageRoot === 'packages/app'), true)
