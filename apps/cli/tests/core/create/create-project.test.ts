  ...
  it('integrates effect-harness as the ai-harness provider under .prelude', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        assert.ok(writeOperations.some(operation => operation.path === 'AGENTS.md' && operation.authority === 'owner'))
...
`)
        ...
        assert.ok(surfaceIds.has('provider-managed-file:effect-harness:AGENTS.md'))
