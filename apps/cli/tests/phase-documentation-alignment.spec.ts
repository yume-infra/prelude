...
function expectDependsOn(content: string, milestoneId: string, dependencies: readonly string[]) {
  for (const dependency of dependencies) {
    ...
    ).toMatch(new RegExp(`${milestoneId}[\\s\\S]{0,240}depends_on[\\s\\S]{0,80}${dependency}`))
  }
}
...
describe('phase documentation alignment', () => {
  ...
  it('keeps Phase 2 handoff connected to smoke gates and the generated scaffold audit skill', () => {
    const phase2 = [
      ...
      readTrackedWorkingDoc(docs.phase2Quality),
    ].join('\n')
  ...
  it('documents M006-M009 with durable dependency language', () => {
    ...
    expect(corpus, 'M009 must remain blocked while its dependencies are incomplete').toMatch(/M009[\s\S]{0,160}(draft|blocking)/)
  })
  ...
  it('states the approved /gsd parallel start scheduling contract', () => {
    ...
    expect(corpus, 'M006 and M008 must be documented as safe to run concurrently after M005').toMatch(/M005[\s\S]{0,120}M006[\s\S]{0,80}M008[\s\S]{0,80}(并行|concurrent|parallel)/i)
    expect(corpus, 'M007 must wait for M006').toMatch(/M007[\s\S]{0,80}(等待|waits?|depends_on)[\s\S]{0,80}M006/i)
    expect(corpus, 'M009 must wait for M006 plus M008').toMatch(/M009[\s\S]{0,120}(M006[\s\S]{0,80}M008|M008[\s\S]{0,80}M006)/i)
  })
})
