...
// @filename: effectFnUntracedImplicitAny.ts
import { Effect } from "effect"

// should trigger: standalone Effect.fnUntraced callback falls back to the helper's any[] args
export const createCallToken = Effect.fnUntraced(function* (input) {
  return input
})
...
export const withTwoParams = Effect.fnUntraced(function* (a, b) {
  return [a, b] as const
})
...
acceptsAny(Effect.fnUntraced(function* (input) {
  return input
}))
...
acceptsString(Effect.fnUntraced(function* (input) {
  return input.length
}))
...
acceptsRequest(Effect.fnUntraced(function* ({ id }) {
  return id
}))
...
make(Effect.fnUntraced(function* (input) {
  return input.length
}))
