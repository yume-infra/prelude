...
function buildGroupLine(selected: RuleEntry | undefined, groups: ReadonlyArray<GroupInfo>, columns: number): string {
  if (!selected) {
    return renderPaddedLine("", "  ", "  ", columns)
  }

  const selectedIndex = groups.findIndex((group) => group.id === selected.group)
  const rotatedGroups = groups.map((_, index) =>
    groups[(selectedIndex + index) % groups.length]!
  )
...
function buildState(
  ...
  return Effect.gen(function*() {
    ...
    const renderedLines = buildFrame(filteredEntries, groups, searchText, severities, normalizedStartIndex, columns)
...
function handleProcess(entries: ReadonlyArray<RuleEntry>, groups: ReadonlyArray<GroupInfo>) {
  return (input: Terminal.UserInput, state: HorizontalListState) => {
    ...
    switch (input.key.name) {
      case "backspace":
        return buildState(entries, groups, 0, state.searchText.slice(0, -1), state.severities).pipe(
          Effect.map((nextState) => Action.NextFrame({ state: nextState }))
        )
      case "left":
        if (filteredEntries.length === 0) return Effect.succeed(Action.Beep())
        return buildState(entries, groups, state.startIndex - 1, state.searchText, state.severities).pipe(
          Effect.map((nextState) => Action.NextFrame({ state: nextState }))
        )
      case "right":
        if (filteredEntries.length === 0) return Effect.succeed(Action.Beep())
      ...
      default:
        ...
        return buildState(entries, groups, 0, state.searchText + getInputText(input)!, state.severities).pipe(
          Effect.map((nextState) => Action.NextFrame({ state: nextState }))
        )
    }
  }
...
function getPromptEntries(rules: ReadonlyArray<RuleInfo>): ReadonlyArray<RuleEntry> {
