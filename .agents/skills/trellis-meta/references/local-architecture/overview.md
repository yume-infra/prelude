...
- `.trellis/`: workflow, tasks, specs, user docs, memory, scripts, and runtime state.
...
2. **Persistence layer**: `.trellis/tasks/`, `.trellis/spec/`, `.trellis/user/`, and `.trellis/workspace/` store tasks, specs, human-facing context, and session memory.
...
| `.trellis/user/` | Human-facing project context docs: project map, reading order, package roles, architecture notes, and common pitfalls. |
...
4. **Put project-specific rules in `.trellis/spec/`, `.trellis/user/`, or a local skill**: executable coding conventions belong in spec; human-facing project context belongs in user docs; do not put team conventions into `trellis-meta`.
...
- To change human-facing project context docs, edit `.trellis/user/` and keep Phase 3 knowledge-doc rules in `workflow.md` aligned.
